...
const ReportInteractionsPage = () => {
  const [activeTab, setActiveTab] = useState("comments");
  const {
    data: reportsCommentData,
    error: reportsCommentError,
    isLoading: reportsCommentIsLoading,
  } = useQuery({
    queryKey: [postKey, "reports"],
    queryFn: async () => await getAllReport(),
  });

  const filteredReports = reportsCommentData?.data.filter((report) => report?.type === ReportType.Comment) || [];

  const tabs = [{ name: "comments", icon: <CommentInteractionsIcon />, label: "COMMENTS" }];
  ...
  return (
    <div className="flex flex-col mx-6">
      ...
      {reportsCommentIsLoading ? (
        <div className="flex items-center justify-center h-[450px]">
          <Spinner size="lg" />
        </div>
      ) : (
        <div className="pt-4 overflow-y-auto max-h-[506px]">
          {activeTab === "comments" && filteredReports && <CommentsComponent data={filteredReports} />}
        </div>
      )}
    </div>
  );
...
const CommentsComponent = ({ data }: { data: ReportWithTypeResponse[] }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center my-8 space-y-4">
        ...
        <span className="font-bold text-2xl">There have been no comments reported on any of the articles</span>
        <span className="text-sm text-[#8e8e8e]">When a comment reports on a post, it will appear here.</span>
      </div>
    );
  }
  return (
          ...
          <div key={report.id} className="flex flex-col my-2 mx-5 border-b">
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col items-end space-y-2">
                <Link
                  href={report.type === ReportType.Reel ? `/r/${report?.post?.id}` : `/p/${report.post?.id}`}
                  className="text-blue-400 text-xs font-bold"
                  onClick={() => handleClick(report)}>
                  Go to {report.type === ReportType.Reel ? "reel" : "post"}
                </Link>
            ...
            <div key={report?.comment?.id} className="flex items-center justify-between my-2.5 py-2.5 ml-10">
              <div className="flex space-x-2">
                ...
                <div className="flex flex-col">
                  <span className="text-sm line-clamp-[1]">
                    <span className="font-bold">{report.user?.username}</span> {report?.comment?.content}
                  </span>
                  <span className="text-xs text-[#737373]">
                    {formatDistanceToNow(report?.comment?.created_at || "", {
                      addSuffix: true,
                    })}
                  </span>
                </div>
...
export default ReportInteractionsPage;
